...
if (!function_exists('coinremitter_wc_gateway_load') && !function_exists('coinremitter_wc_action_links'))
{
	...
	function coinremitter_wc_gateway_load(){
		...
		class WC_Gateway_CoinRemitter extends WC_Payment_Gateway
		{
			...
			private function coinremitter_settings()
			{
				...
				$this->description = $this->description.$this->coinremitterSetPaymnetOptDesc();	
			...
			public function coinremitterSetPaymnetOptDesc(){
				global $coinremitter;
				global $woocommerce;
				$total_amt = 0;
				$currancy_type = get_woocommerce_currency();
	...
	function coinremitter_add_wallet(){
		...
		if($ConResp['flag'] != 1){
			...
			if($MinInvoiceValue == ''){
				...
				$Result['msg'] = 'Minimum invoice value must be greater than 0.00001';
				$Result['flag'] = 0;
				echo wp_json_encode($Result);
				die;
			...
			if(!file_exists($permfile)){
				$url = COINREMITTER_API_URL_ALL_COIN."assets/img/coins/".$file_name;
				if (@getimagesize($url)) {
